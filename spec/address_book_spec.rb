...
RSpec.describe AddressBook do
	...
	describe "#import_from_csv_2" do
      ...
      expect(book_size).to eq 3
		...
		it "imports the 1st entry" do
			book.import_from_csv("entries_2.csv")
			entry_one = book.entries[0]
			check_entry(entry_one, "Alice Ray", "555-555-9588", "alice@gmail.com")
		end
		
		it "imports the 2nd entry" do
      book.import_from_csv("entries_2.csv")
      entry_two = book.entries[1]
      check_entry(entry_two, "Marsha", "555-555-9695", "marsha@gmail.com")
    end
 
    it "imports the 3rd entry" do
      book.import_from_csv("entries_2.csv")
      entry_three = book.entries[2]
      check_entry(entry_three, "Zoe", "555-555-4104", "zoe@gmail.com")
    end
	end
	...
	describe "#interactive_search" do
		it "searches AddressBook for a non-existent entry" do
			book.import_from_csv("entries.csv") 
			entry = book.interactive_search("Dan")
			expect(entry).to be_nil
		end

		it "searches AddressBook for Bill" do
			book.import_from_csv("entries.csv")
			entry = book.interactive_search("Bill")
			expect(entry).to be_a Entry
			check_entry(entry, "Bill", "555-555-4854", "bill@blocmail.com")
		end 
		...
		it "searches AddressBook for Bob" do
			book.import_from_csv("entries.csv")
			entry = book.interactive_search("Bob")
			expect(entry).to be_a Entry
			check_entry(entry, "Bob", "555-555-5415", "bob@blocmail.com")
		end
		...
		it "searches AddressBook for Joe" do
			book.import_from_csv("entries.csv")
			entry = book.interactive_search("Joe")
			expect(entry).to be_a Entry
			check_entry(entry, "Joe", "555-555-3660", "joe@blocmail.com")
		end
		...
		it "searches AddressBook for Sally" do
			book.import_from_csv("entries.csv")
			entry = book.interactive_search("Sally")
			expect(entry).to be_a Entry
			check_entry(entry, "Sally", "555-555-4646", "sally@blocmail.com")
		end
		...
		it "searches AddressBook for Sussie" do
			book.import_from_csv("entries.csv")
			entry = book.interactive_search("Sussie")
			expect(entry).to be_a Entry
			check_entry(entry, "Sussie", "555-555-2036", "sussie@blocmail.com")
		end
